...
class Image(object):
    ...
    def get_random_loc(self, radius):
        locx = random.randrange(radius, self.image_shape[1] - radius)
        locy = random.randrange(radius, self.image_shape[0] - radius)
        return locx, locy
    ...
    def create_image(self, rust_path, bg_path, sigmas_x = (10, 20, 40, 60, 100), sigmas_y = (10, 20, 40, 60, 100), 
        ...
        for _ in range(num_locs):
            ...
            num_sec_locs = random.choice((1,2,3))

            for _ in range(num_sec_locs):
                sigma_x = random.choice(sigmas_x)
                sigma_y = random.choice(sigmas_y)

                sec_loc_x, sec_loc_y = self.get_random_point(locx, locy, sigma_x + 20, sigma_y + 20, radius)
                
                ptsx2, ptsy2 = self.get_point_list(sigma_x, sigma_y+10, radius, num_points, kernel, sec_loc_x, sec_loc_y)
                mask = self.draw_dots(ptsx2, ptsy2, radius - 1, mask)
                #mask = self.morph_dots(kernel, mask)

            #Adding some noise
            ptsx2, ptsy2 = self.get_point_list(sigma_x, sigma_y+10, radius, num_points, kernel, locx, locy)
            mask = self.draw_dots(ptsx2, ptsy2, radius - 1, mask)

            ptsx3, ptsy3 = self.get_point_list(sigma_x, sigma_y+20, radius, num_points*2, kernel, locx, locy)
...
def test(RUST_DIR, BG_DIR):
    ...
    image = Image(1)
    image.create_image('{}/{}'.format(RUST_DIR, rust), '{}/{}'.format(BG_DIR, bg),
                    sigmas_x = (5, 20, 30), sigmas_y = (5, 20, 30), radius = 3, num_locs=num_locs, num_points = 2000, 
                    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (20, 30)))
